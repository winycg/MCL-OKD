...
__all__ = ['one_resnet32', 'one_resnet110']
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, number_net=4, num_classes=100, zero_init_residual=False,
        ...
        self.gate_fc = nn.Linear(32 * block.expansion, self.number_net)
        self.gate_norm = nn.BatchNorm1d(self.number_net)
    ...
    def _forward(self, input):
        ...
        for i in range(self.number_net):
            ...
            logits.append(x)
        branch_weights = F.softmax(F.relu(self.gate_norm(self.gate_fc(torch.flatten(self.avgpool(input), 1)))),
                                   dim=1)
        t_logits = 0.
        for id, logit in enumerate(logits):
            t_logits = t_logits + branch_weights[:, id].unsqueeze(1) * logit
