...
__all__ = ['cl_ilr_resnet32', 'cl_ilr_resnet110']
...
class aux_classifier(nn.Module):
    def __init__(self, block, inplanes, planes, groups, base_width, num_classes):
        ...
        downsample3 = nn.Sequential(
            conv1x1(inplanes, planes*block.expansion, stride=2),
            nn.BatchNorm2d(planes*block.expansion),
        )
        self.module3 = block(inplanes, planes, stride=2, downsample=downsample3,
...
class vanilla_classifier(nn.Module):
...
class ResNet(nn.Module):
    ...
    def _forward(self, input):
        ...
        for i in range(0, math.ceil(self.number_net / 2)):
            x = getattr(self, 'layer2_' + str(i))(inputs)
        ...
        for i in range(self.number_net):
            x = getattr(self, 'layer3_' + str(i))(x2[i])
            x = self.avgpool(x)
            embedding.append(x)
            x = x.view(x.size(0), -1)
...
def cl_ilr_resnet32(**kwargs):
    return _resnet(BasicBlock, [5, 5, 5], **kwargs)
'''
def cl_ilr_resnet110(**kwargs):
...
if __name__ == '__main__':
    net = cl_ilr_resnet32(num_classes=100)
