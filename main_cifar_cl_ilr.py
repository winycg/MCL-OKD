...
parser.add_argument('--arch', default='cl_ilr_resnet32', type=str, help='network architecture')
...
def train(epoch, criterion_list, optimizer):
    ...
    for batch_idx, (input, target) in enumerate(trainloader):
        ...
        for i in range(len(logits)):
            loss_cls = loss_cls + 0.5 * criterion_cls(logits[i], target)
        for i in range(len(logits)):
            ensemble_logits = 0.
            for j in range(len(logits)):
                if i != j:
                    ensemble_logits = ensemble_logits + logits[j]
            ensemble_logits = ensemble_logits / (len(logits) - 1)
            ensemble_logits = ensemble_logits.detach()
            loss_div = loss_div + 0.5 * criterion_div(logits[i], ensemble_logits)
