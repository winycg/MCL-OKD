...
parser.add_argument('--arch', default='okddip_resnet32', type=str, help='network architecture')
...
if args.resume is False:
    with open('result/' + str(os.path.basename(__file__).split('.')[0]) +
              args.arch + '_' + str(args.num_branches) + '_' +
              str(args.manual_seed) + '.txt', 'a+') as f:
        f.seek(0)
        f.truncate()
...
def train(epoch, criterion_list, optimizer):
    ...
    if epoch < consistency_epoch:
        ...
        consistency_weight = get_current_consistency_weight(epoch - consistency_epoch, 80)
    for batch_idx, (input, target) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        output_batch, x_m, x_stu = net(input)
        ...
        for i in range(args.num_branches):
            if i == args.num_branches - 1:
                _, predicted = x_stu.max(1)
            else:
                _, predicted = output_batch[:, :, i].squeeze(-1).max(1)
...
def test(epoch, criterion_cls, criterion_div):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, target) in enumerate(testloader):
            ...
            output_batch, x_m, x_stu = net(inputs)
            ...
            loss = loss_true + criterion_cls(x_stu, target) + 1.0 * (
                    loss_group + criterion_div(x_stu, torch.mean(output_batch, dim=2)))
            ...
            for i in range(args.num_branches):
                if i == args.num_branches-1:
                    ensemble_logit = ensemble_logit + x_stu
                else:
                    ensemble_logit = ensemble_logit + output_batch[:, :, i].squeeze(-1)

            for i in range(args.num_branches+1):
                if i == args.num_branches-1:
                    _, predicted = x_stu.max(1)
                elif i == args.num_branches:
                    _, predicted = ensemble_logit.max(1)
                else:
                    _, predicted = output_batch[:, :, i].squeeze(-1).max(1)
                correct[i] += predicted.eq(target).sum().item()
                total[i] += target.size(0)
        ...
        with open('result/' + str(os.path.basename(__file__).split('.')[0]) +
                  args.arch + '_' + str(args.num_branches) + '_' +
                  str(args.manual_seed) + '.txt', 'a+') as f:
            f.write('test epoch:{0}\t accuracy:{1}\n'
